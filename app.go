...
// Struct Metadata yang disimpan di file metadata.json
type BookMetadata struct {
	Tags        []string `json:"tags"`
	Description string   `json:"description"`
	MaskCover   bool     `json:"mask_cover"` // Jika true, cover disensor di library
...
func HashString(s string) string {
	h := sha256.New(); h.Write([]byte(s)); return hex.EncodeToString(h.Sum(nil))
}
...
func (a *App) ToggleHiddenZone(p string) bool {
	if a.config.HiddenZoneHash == "" {
		a.SetHiddenZonePassword(p); a.hiddenModeActive = true; return true
	}
	if HashString(p) == a.config.HiddenZoneHash {
		a.hiddenModeActive = true; return true
	}
	return false
}
...
func (a *App) UpdateBookMetadata(bookName, newName, description string, tags []string, isHidden, maskCover bool) error {
	...
	for _, t := range tags {
		...
		if l != "" && !uniqueMap[l] { uniqueMap[l] = true; cleanTags = append(cleanTags, l) }
	}

	// 3. Simpan Metadata Baru
	meta := BookMetadata{
		Tags:        cleanTags,
...
func (a *App) CreateBook(bookName string, sourcePath string, syncMode bool) string {
	if bookName == "" || sourcePath == "" { return "Data kosong" }
	...
	if !syncMode {
		if _, err := os.Stat(bookPath); !os.IsNotExist(err) { return "Buku sudah ada." }
	}
	...
	filepath.WalkDir(sourcePath, func(path string, d fs.DirEntry, err error) error {
		...
		if ext == ".jpg" || ext == ".png" || ext == ".jpeg" || ext == ".webp" {
			...
			if err == nil {
				if srcImg.Bounds().Dx() > maxWidth { srcImg = imaging.Resize(srcImg, maxWidth, 0, imaging.Lanczos) }
...
func (a *App) GetBooks() []Book {
	...
	for _, e := range entries {
		if e.IsDir() {
			...
			// Logic Hidden
			if meta.IsHidden && !a.hiddenModeActive { continue }

			// Logic Lock
			isLocked := false
			if _, err := os.Stat(filepath.Join(bookPath, "security.json")); err == nil { isLocked = true }
				...
				if d, err := os.ReadFile(filepath.Join(bookPath, "cover.db")); err == nil {
					...
					filepath.WalkDir(bookPath, func(p string, d fs.DirEntry, er error) error {
						if !d.IsDir() && strings.HasSuffix(strings.ToLower(d.Name()), ".jpg") {
							coverName, _ = filepath.Rel(bookPath, p); return io.EOF
						}
						return nil
					})
				}
			...
			books = append(books, Book{
				Name: e.Name(), Cover: coverB64, Tags: meta.Tags, 
				Description: meta.Description, IsLocked: isLocked, 
				IsHidden: meta.IsHidden, MaskCover: meta.MaskCover,
			})
		}
	}
	return books
...
// ... (Functions GetChapters, GetImages, Crypto, SetBookCover, dll SAMA SEPERTI SEBELUMNYA) ...
// Pastikan copy function helper (Encrypt/Decrypt, natsort, GetImagesInChapter) dari kode sebelumnya ada di sini.
// Untuk menghemat ruang, saya asumsikan helper functions tersebut tetap ada di bawah sini.

func (a *App) GetChapters(bookName string) []string {
	var chapters []string
	entries, _ := os.ReadDir(filepath.Join(a.vaultDir, SanitizeName(bookName)))
	for _, e := range entries { if e.IsDir() { chapters = append(chapters, e.Name()) } }
	natsort(chapters)
	return chapters
}
func (a *App) GetImagesInChapter(bookName, chapterName string) []string {
	...
	if chapterName != "" { targetPath = filepath.Join(targetPath, chapterName) }
	entries, _ := os.ReadDir(targetPath)
	for _, e := range entries {
		if !e.IsDir() && strings.HasSuffix(strings.ToLower(e.Name()), ".jpg") { files = append(files, e.Name()) }
	}
	natsort(files)
	return files
}
func EncryptData(plain []byte) ([]byte, error) {
	block, _ := aes.NewCipher(EncryptionKey); gcm, _ := cipher.NewGCM(block); nonce := make([]byte, gcm.NonceSize()); io.ReadFull(rand.Reader, nonce); return gcm.Seal(nonce, nonce, plain, nil), nil
}
func TryDecryptData(data []byte) []byte {
	if len(data) < 16 { return data }
	block, _ := aes.NewCipher(EncryptionKey); gcm, _ := cipher.NewGCM(block); nonce, ciphertext := data[:gcm.NonceSize()], data[gcm.NonceSize():]; plain, err := gcm.Open(nil, nonce, ciphertext, nil); if err != nil { return data }; return plain
}
func (a *App) SetBookCover(b, i string) error { return os.WriteFile(filepath.Join(a.vaultDir, SanitizeName(b), "cover.db"), []byte(i), 0644) }
func (a *App) DeleteBook(b string) error { return os.RemoveAll(filepath.Join(a.vaultDir, SanitizeName(b))) }
func (a *App) DeleteImage(b, i string) error { return os.Remove(filepath.Join(a.vaultDir, SanitizeName(b), i)) }
