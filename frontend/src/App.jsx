...
function App() {
    ...
    const handleToggleHiddenZone = async () => {
        ...
        if (!pass) return;
        const success = await ToggleHiddenZone(pass);
        if (success) await fetchBooks(); else alert("Password Salah!");
    };
    ...
    const openEditModal = (e, book) => {
        ...
        setEditLockPass(''); setEditIsHidden(book.is_hidden); setEditMaskCover(book.mask_cover);
    };
    ...
    const handleUnlockAction = async () => { if(confirm("Hapus proteksi password?")) { await UnlockBook(editingBook.name); setEditingBook(null); await fetchBooks(); } };
    const handleReaderSetCover = async (filename) => {
        if(!currentBookObj || !filename) return;
        let targetFile = filename; if (currentChapter) targetFile = currentChapter + "/" + filename;
                        ...
                        {uniqueTags.map(tag => {
                            ...
                            return (
                                <button key={tag} className={`nav-item tag-item ${status || ''}`} onClick={() => toggleTag(tag)}>
                                    ...
                                    {status === 'include' && <CheckIcon />}
                                    {status === 'exclude' && <CrossIcon />}
                                </button>
                            );
                        })}
                    </div>
            ...
            <div className="chapter-list-container">
                ...
                <div className="chapter-list">
                    {chapters.map(chapter => (
                        <div key={chapter} className="chapter-item" onClick={() => handleOpenChapter(currentBookObj.name, chapter)}>
                            <div className="chapter-icon"><FolderIcon /></div>
                            <div className="chapter-name">{chapter.replace(/_/g, ' ')}</div>
                    ...
                    <div key={b.name} className="book-card" style={{opacity: b.is_hidden ? 0.7 : 1, border: b.is_hidden ? '1px dashed #f38ba8' : 'none'}}>
                        <div className="book-cover" onClick={() => handleOpenBook(b)}>
                            {b.cover ? <img src={b.cover} alt="cover"/> : (
                                <div className="book-cover-placeholder" style={{flexDirection:'column'}}>
                                    {b.mask_cover ? <><EyeOffIcon style={{width:40,height:40}}/><span style={{fontSize:12, marginTop:10}}>Cover Hidden</span></> : "📚"}
                                </div>
                            )}
                ...
                <div className="top-bar">
                    <h2>
                        {view === 'library' ? 'My Collection' : 
                         view === 'chapters' ? currentBookObj?.name.replace(/_/g, ' ') : 
                         currentChapter ? currentChapter.replace(/_/g, ' ') : currentBookObj?.name.replace(/_/g, ' ')}
                    </h2>
